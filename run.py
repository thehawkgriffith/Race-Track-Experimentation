...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        return np.array([t for t in states]), np.array(actions), np.array(rewards, np.float32), np.array(dones, np.uint8), np.array([t for t in next_states])
...
def main():
    ...
    net = DQN(env.observation_shape, env.n_actions).to(device)
    tgt_net = DQN(env.observation_shape, env.n_actions).to(device)
    print(net)
